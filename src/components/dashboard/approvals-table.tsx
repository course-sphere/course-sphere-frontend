...
export function ApprovalsTable({ requests }: Props) {
    ...
    // State "tà đạo": dùng để ẩn đi những khóa học đã được Admin duyệt
    const [hiddenIds, setHiddenIds] = useState<Set<string>>(new Set());

    const handleApprove = async () => {
        if (!selectedReq) return;

        // Hiện Toast tung hoa xanh rực rỡ
        toast.success(
            `Course "${selectedReq.courseTitle}" successfully approved and published!`,
        );

        // Cho nó "bốc hơi" khỏi UI ngay lập tức
        ...
        <div className="border-border/60 bg-card overflow-hidden rounded-xl border shadow-sm">
            <div className="overflow-x-auto">
                <Table>
                    ...
                    <TableBody>
                        {requests
                            .filter((req) => !hiddenIds.has(req.id)) // Lọc bỏ những row đã duyệt
                            .map((req) => (
                                <TableRow
                                    key={req.id}
