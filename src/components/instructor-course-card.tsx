...
export function InstructorCourseCard({ course }: Props) {
    ...
    return (
        <>
                        ...
                        {course.status === 'draft' && (
                            <>
                                <Button asChild size="sm">
                                    <Link
                                        href={`/course/create?id=${course.id}`}
                                    >
                                ...
                                <Button asChild variant="secondary" size="sm">
                                    <Link
                                        href={`/preview?materialId=${course.id}`}
                                    >
                            ...
                            <>
                                ...
                                <Button asChild variant="outline" size="sm">
                                    <Link
                                        href={`/course/create?id=${course.id}`}
                                    >
                                        <Edit className="mr-2 h-4 w-4" /> Fix
                                        Issues
                                    </Link>
                                </Button>
                            </>
            ...
            <ConfirmDialog
                open={isReasonOpen}
                onOpenChangeAction={setIsReasonOpen}
                title="Review Feedback"
                description={
                    course.rejectReason || 'No specific reason provided.'
                }
                confirmText="Got it"
                cancelText="Close"
                onConfirmAction={() => setIsReasonOpen(false)}
            />
        </>
    );
}
